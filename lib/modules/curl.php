		...
		public function output(){
			...
			if(function_exists('curl_init')){
				try{
					...
					if(get_transient('sv_proven_expert') && 1==2){
						...
						if($data['status'] == 'success'){
							...
							set_transient('sv_proven_expert', json_decode($json), 86400);
					...
					@file_put_contents(dirname($cachePath) . $errorFile, $errorMessage);
					$output											= ('<!-- exception error [v' . $this->core->version . '] -->');
				}
			}else{
				$output												= ('<!-- no curl package installed [v' . $this->core->version . '] -->');
			}
